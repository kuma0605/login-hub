<template>
  <div
    aria-hidden="true"
    class="pointer-events-none fixed inset-0 overflow-hidden select-none bg-[#eff2fb]"
  >
    ...
    <div class="mask-fade-edges absolute inset-0">
      <span
        v-for="trace in horizontalTraces"
        :key="'h-' + trace.offset"
        class="animate-trace-x absolute left-0 h-[2px] rounded-full"
        :style="{
          top: trace.offset,
          width: trace.length,
      ...
      <span
        v-for="trace in verticalTraces"
        :key="'v-' + trace.offset"
        class="animate-trace-y absolute top-0 w-[2px] rounded-full"
        :style="{
          left: trace.offset,
          height: trace.length,
          animationDelay: trace.delay,
      ...
      <span
        ...
        class="animate-node absolute h-2 w-2 rounded-full bg-[#38bdf8]"
        :style="{
          left: node.left,
          top: node.top,
    ...
    <div class="mask-fade-edges absolute inset-0">
      <div
        class="animate-sheen absolute -inset-y-1/2 left-[32%] w-[30rem] -translate-x-1/2"
        :style="{
          background:
    ...
    <div class="absolute inset-0">
      <div
        class="animate-bloom absolute left-[28%] top-[-20rem] h-[42rem] w-[42rem] rounded-full"
        :style="{
          background:
      ...
      />
      <div
        class="animate-bloom-alt absolute bottom-[-16rem] left-[-12rem] h-[32rem] w-[32rem] rounded-full"
        :style="{
          background:
      ...
      />
      <div
        class="animate-bloom-alt absolute bottom-[-18rem] right-[-10rem] h-[34rem] w-[34rem] rounded-full"
        :style="{
          animationDelay: '3.5s',
          background:
    ...
    <div class="absolute inset-x-0 bottom-0 h-40 bg-gradient-to-b from-transparent to-white/40 pointer-events-none" />
  </div>
</template>

<script>
export default {
  name: 'SmartSecurityBackground',
  data() {
    return {
      horizontalTraces: [
...
<style scoped>
/* Smart Security — all animation CSS self-contained, no global index.css dependency */

/* Renamed with ss- prefix to avoid collisions when copied to other projects */
.ss-mask-fade-edges {
...
.animate-bloom     { animation: ss-bloom-breathe 6.5s ease-in-out infinite; }
.animate-bloom-alt { animation: ss-bloom-breathe-alt 8s ease-in-out infinite; }
.animate-trace-x   { animation: ss-trace-x 4.2s linear infinite; }
.animate-trace-y   { animation: ss-trace-y 5s linear infinite; }
.animate-sheen     { animation: ss-sheen-sweep 7s ease-in-out infinite; }
.animate-node      { animation: ss-node-pulse 3.2s ease-in-out infinite; }
