...
export default {
  ...
  computed: {
    latestThroughput() {
      return this.throughput[this.throughput.length - 1];
    },
    pts() {
      return this.throughput.map((v, i) => {
        const x = (i / (this.throughput.length - 1)) * this.w;
        const y = this.h - (v / 100) * this.h;
    ...
    areaPath() {
      return `${this.linePath} L ${this.w},${this.h} L 0,${this.h} Z`;
    }
  },
  mounted() {
    this.timer = setInterval(() => {
      const list = this.throughput;
      const last = list[list.length - 1];
      const drift = (Math.random() - 0.48) * 12;
      const next = Math.min(98, Math.max(30, last + drift));
      this.throughput = [...list.slice(1), next];
    }, 1200);
  },
  beforeDestroy() {
    if (this.timer) {
  ...
  methods: {
    handleSubmit() {
      ...
      if (!this.operator.includes('@')) {
        this.error = 'Operator ID must be a valid credentialed address.';
        return;
      }
      if (this.key.length < 6) {
        this.error = 'Access key must be at least 6 characters.';
        return;
      }
      this.error = null;
